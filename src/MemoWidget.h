...
const QHash<int, QString> COLOR_TABLE{{0, "red"},
                                      {1, "purple"},
                                      {2, "blue"},
                                      {3, "green"},
                                      {4, "brown"}};
...
public:
    explicit MemoWidget(MemoInfo *memoInfo, bool isEditMode,
                        QWidget *parent = 0);
    ~MemoWidget();
    ...
    void setMode(
            Mode mode);  //切换便签显示模式，
                         // VIEW：窗口未激活时的便签查看模式；SELECT：窗口激活但未进入编辑状态模式；EDIT：编辑模式
